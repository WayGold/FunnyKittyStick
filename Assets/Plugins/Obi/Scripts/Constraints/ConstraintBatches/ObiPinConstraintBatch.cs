...
public class ObiPinConstraintBatch : ObiConstraintBatch
{
	...
	public void UpdateColliderIndices(ObiColliderGroup group){
		...
		// if not in solver, do nothing:
		if (solverIndices == null) return;
		...
		for (int i = 0; i < pinOffsets.Count; i++)
		{
			...
			if (group != null) {
				...
				// if the collider does not exist in the collider group, insert it at the end.
				if (colliderIndex == -1){
					colliderIndex = group.colliders.Count;
					group.colliders.Add(pinBodies[i]);
				}
