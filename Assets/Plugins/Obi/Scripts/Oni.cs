...
public static class Oni {
	...
	public struct HeightData{
		...
		public void UpdateHeightData(){
			...
			float[] buffer = new float[source.terrainData.heightmapResolution * source.terrainData.heightmapResolution];
			for (int y = 0; y < source.terrainData.heightmapResolution; ++y)
				for (int x = 0; x < source.terrainData.heightmapResolution; ++x)
					buffer[y*source.terrainData.heightmapResolution+x] = heights[y,x];
	...
	public struct HeightmapShape{
		...
		public HeightmapShape(TerrainCollider source, IntPtr data){
			resolutionU = source.terrainData.heightmapResolution;
			resolutionV = source.terrainData.heightmapResolution;
